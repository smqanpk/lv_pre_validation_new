...
def lv_ug_hanging(arr_lv_ug_exclude_geom, arr_lv_oh_exclude_geom):
        ...
        if device_temp not in arr_lv_ug_exclude_geom:
            ...
            if geom_g:
                ...
                for g1 in range(len(polyline_y_g)):
                    # 25.2.2021: remove exclusive check: will add all points, including duplicate ones
    ...
    if feat_count > lv_ug_max_count:
        ...
        for f in feat:
            device_id = f.attribute('device_id')
            if device_id not in arr_lv_ug_exclude_geom:
            #if device_id == 'RPS6122ugc70832': #for testing
                geom = f.geometry()
                if geom:
                    y = geom.mergeLines()
                    polyline_y = y.asPolyline()
                    ...
                    for v_point in arr_point:
                        ...
                        if bool_test:
                            ...
                            if distance_v_one <= 0.001:
                                #print('v one is ' + str(v_one))
                                #print('lv point is ' + str(v_point))
                                #print('distance_v_one is ' + format(distance_v_one,'.9f') + 'm')
                        ...
                        if bool_test:
                            ...
                            if distance_v_last <= 0.001:
                                ...
                                #print('lv point is ' + str(v_point))
                                #print('distance_v_last is ' + format(distance_v_last,'.9f') + 'm')
                                arr_snap_v_last.append(device_id)
                        if len(arr_snap_v_one) > 2 and len(arr_snap_v_last) > 2:
                            break  # break out of for loop
                    if len(arr_snap_v_one) <= 1 or len(arr_snap_v_last) <= 1:
                        # print('len arr_snap_v_one = ', len(arr_snap_v_one))
                        # print('len arr_snap_v_last = ', len(arr_snap_v_last))
                        # print('device id: ', device_id, ' has ', len(arr_snap_v_one) , ' len(v one); ', len(arr_snap_v_last), ' len(v last);')
                        arr.append(device_id)
